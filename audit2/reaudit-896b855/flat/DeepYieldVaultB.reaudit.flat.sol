...
library VaultBDepositLib {
    ...
    /// @notice Tolerant canonical commitment proof used only to materialize a
    /// missing local recovery snapshot. The pinned direct asset source is Main
    /// in the shipped graph and remains queryable when the Adapter is down.
    function redeemCycleCommittedForRecovery(IVaultBAsyncStrategy strategy, address assetSource, bool localCommitted)
        external
        view
        returns (bool)
    {
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    function forceSettleStuckCycle() external nonReentrant {
        if (!_redeemCycleCommitted) {
            if (
                !VaultBDepositLib.redeemCycleCommittedForRecovery(strategy, strategyAssetSource, false)
                    || outstandingRedeemCount == 0
            ) revert RedeemCycleNotCommitted();
            _takeRedeemCycleRecoverySnapshot();
            return;
        }
    ...
    function _takeRedeemCycleRecoverySnapshot() internal {
        _writeRedeemCycleSnapshot(0, _spendableIdle());
    }

...
contract VaultBPriceGuard is AccessControlDefaultAdminRules, IVaultBPriceGuard {
    ...
    /// @inheritdoc IVaultBPriceGuard
    function recoveryClosePolicy(uint256 assetExpected, uint256 pairedExpected)
        external
        view
        returns (uint256 referenceUsdtPerWbnb, uint256 fullNotional, uint16 selectedLossBps, bool emergencyBudgetUsed)
    {
        // A zero-value stage has no execution value to lose and must not be
        // stranded merely because an oracle is unavailable.
        if (assetExpected == 0 && pairedExpected == 0) return (0, 0, normalLossBps, false);

        uint256 chainlinkReference = _chainlinkQuote(WBNB, 1e18);
        uint256 twapReference = _twapQuote(WBNB, USDT, 1e18);
        if (chainlinkReference == 0 || twapReference == 0) revert InvalidAmount();

        uint256 upperReference;
        if (chainlinkReference < twapReference) {
            referenceUsdtPerWbnb = chainlinkReference;
            upperReference = twapReference;
        } else {
            referenceUsdtPerWbnb = twapReference;
            upperReference = chainlinkReference;
        }

        uint256 deviationBps =
            FullMath.mulDivRoundingUp(upperReference - referenceUsdtPerWbnb, BPS, referenceUsdtPerWbnb);
        uint256 pairedNotional;
        if (pairedExpected != 0) {
            uint256 pairedChainlink = _chainlinkQuote(WBNB, pairedExpected);
            uint256 pairedTwap = _twapQuote(WBNB, USDT, pairedExpected);
            pairedNotional = pairedChainlink > pairedTwap ? pairedChainlink : pairedTwap;
        }
        fullNotional = assetExpected + pairedNotional;

        // This recovery-specific view deliberately treats an absent/expired
        // widening allocation as normal policy. Unexpected guard/oracle
        // failures are not caught or downgraded. Active-but-insufficient
        // capacity already degrades to normal in `_lossBudget` (Round 8).
        if (emergencyLossBps == 0 || emergencyExpiresAt <= block.timestamp) {
            selectedLossBps = normalLossBps;
        } else {
            (selectedLossBps, emergencyBudgetUsed) = _lossBudget(true, fullNotional);
        }

        uint256 deviationLimit = emergencyBudgetUsed ? maxEmergencyOracleDeviationBps : maxOracleDeviationBps;
        if (deviationBps > deviationLimit) {
            revert OracleDeviation(chainlinkReference, twapReference, deviationBps);
        }
    }

    /// @notice Called by an explicitly authorized execution adapter after its
    /// router/balance checks. A later revert rolls the swap and this debit back
    /// atomically, so failed attempts never consume the window. A non-swap LP
    /// decrease may read the budget policy but deliberately does not debit it.

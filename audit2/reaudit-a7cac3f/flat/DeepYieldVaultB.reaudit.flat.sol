...
library VaultBDepositLib {
    ...
    function activateCandidate(
        ...
        if (address(oldStrategy) != address(0)) {
            ...
            (bool prepared, bytes memory data) =
                address(oldStrategy).call(abi.encodeWithSelector(PREPARE_MIGRATION_SELECTOR));
            bool attested = prepared && data.length == 32 && abi.decode(data, (bool));
            if (!attested) {
                try oldStrategy.estimatedTotalAssets() returns (uint256 assets) {
                    if (assets != 0) revert StrategyNotEmpty();
                } catch {
    ...
    function redeemCommitRoute(
        uint256 thresholdBase,
        ...
        uint256 available
    ) external pure returns (uint256 threshold, bool subEconomic, bool idleCovers) {
        threshold = _commitThreshold(thresholdBase, minimumShares);
        if (outstandingShares >= threshold) return (threshold, false, false);
        subEconomic = true;
        idleCovers = available >= Math.mulDiv(assetsSnapshot, committedShares, supplySnapshot);
    }

    function _commitThreshold(uint256 thresholdBase, uint256 minimumShares) private pure returns (uint256 threshold) {
        threshold = Math.ceilDiv(thresholdBase, 20);
        if (threshold < minimumShares) threshold = minimumShares;
    }

    function redeemSettlementPayout(
        uint256 assetsSnapshot,
        uint256 committedShares,
        uint256 supplySnapshot,
        uint256 available,
        bool strategyHealthy
    ) external pure returns (uint256 payout, uint256 batchShare) {
        batchShare = Math.mulDiv(assetsSnapshot, committedShares, supplySnapshot);
        uint256 cap = strategyHealthy ? available : Math.mulDiv(available, committedShares, supplySnapshot);
        payout = batchShare < cap ? batchShare : cap;
    }
        ...
        returns (bool)
    {
        ...
        (bool responsive, bytes memory response) = address(strategy).staticcall{gas: probeGas}(
            abi.encodeWithSelector(IVaultBAsyncStrategy.withdrawalCycleCommitted.selector)
        );
        return !responsive || response.length != 32;
    }
    ...
    function coveredRedeemShares(
        uint256 requestShares,
        uint256 payoutAssets,
        uint256 assetsSnapshot,
        uint256 committedShares,
        uint256 supplySnapshot
    ) external pure returns (uint256 burnedShares) {
        uint256 batchShare = Math.mulDiv(assetsSnapshot, committedShares, supplySnapshot);
        if (batchShare == 0) return 0;
        burnedShares = Math.mulDiv(requestShares, payoutAssets, batchShare);
        if (burnedShares > requestShares) burnedShares = requestShares;
    }

    function previewRedeemCycleSettlement(
        IVaultBAsyncStrategy strategy,
        uint256 supply,
        uint256 batchShares,
        uint256 assetsSnapshot,
        ...
        uint256 minimumResidualAssets,
        uint16 maximumLossBps
    ) external view returns (bool valid, uint256 payout) {
        return _previewRedeemCycleSettlement(
            strategy,
            supply,
            batchShares,
            assetsSnapshot,
    ...
    function claimableRedeemRequest(ClaimableRedeemInput memory p) external view returns (uint256) {
        if (p.forceSettled) {
            if (p.requestShares == p.outstandingShares) return p.payoutAssets - p.payoutClaimed;
            return Math.mulDiv(p.payoutAssets, p.requestShares, p.committedShares);
        }
        if (address(p.strategy) == address(0) || !p.strategy.withdrawalReady(p.strategyRequestId)) return 0;
        bool committed = p.localCommitted || p.strategy.withdrawalCycleCommitted();
        uint256 currentAssets = p.asset.balanceOf(p.vault) + p.strategy.estimatedTotalAssets() - p.claimableAssets;
        if (!committed) {
            return Math.mulDiv(p.requestShares, currentAssets + 1, IERC20(p.vault).totalSupply() + 1e6);
        }

        uint256 supply = p.localCommitted ? p.committedSupply : IERC20(p.vault).totalSupply();
        uint256 batchShares = p.localCommitted ? p.committedShares : p.outstandingShares;
        uint256 payout = p.payoutAssets;
        if (!p.settlementInitialized) {
            (bool valid, uint256 previewPayout) = _previewRedeemCycleSettlement(
                p.strategy,
                supply,
                batchShares,
                p.localCommitted ? p.assetsSnapshot : currentAssets,
                currentAssets,
                p.protocolCredit,
                p.minimumResidualAssets,
                p.maximumLossBps
            );
            if (!valid) return 0;
            payout = previewPayout;
        }
        if (p.requestShares == p.outstandingShares && p.settlementInitialized) return payout - p.payoutClaimed;
        return Math.mulDiv(payout, p.requestShares, batchShares);
    }
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    /// @notice Supply basis frozen when the redeem queue opens. Both readiness
    /// and economic routing use it, so later deposits cannot move the threshold.
    /// Zero while the queue is empty.
    ...
    function _activateStrategy(address newStrategy, address expectedSource) internal {
        ...
        address source = VaultBDepositLib.activateCandidate(
            IERC20(asset()), address(this), strategy, strategyAssetSource, candidate, expectedSource
        );
        strategy = candidate;
    ...
    function releaseDeferredRedeemHandle(bytes32 deferredId) external onlyRole(ADMIN_ROLE) {
        if (!deferredRedeemHandles[deferredId]) revert RedeemHandleNotDeferred(deferredId);
        VaultBDepositLib.cancelWithdrawal(strategy, strategyAssetSource, deferredId);
        delete deferredRedeemHandles[deferredId];
        unchecked {
            _deferredRedeemHandleCount -= 1;
        }
        emit RedeemHandleReleased(deferredId);
    }
    ...
    function _tryAutoCommit(IVaultBAsyncStrategy activeStrategy) internal {
        if (outstandingRedeemCount < redeemCycleMaxPendingAtOpen) return;
    ...
    /// @notice Commit a batch only after it reaches 5% of queue-open supply. There
    ...
    function _commitRedeemCycle(IVaultBAsyncStrategy activeStrategy, uint256 threshold) internal {
        ...
        // Economic routing uses the same queue-open frozen basis as the public
        // readiness view. A later deposit cannot move an honest batch below the
        // unwind threshold; genuinely-small batches remain idle-only.
        uint256 available = _spendableIdle();
        (uint256 routingThreshold, bool subEconomic, bool idleCovers) = VaultBDepositLib.redeemCommitRoute(
            redeemCycleThresholdBase,
            MIN_REDEEM_SHARES,
        ...
        if (subEconomic) {
            ...
            // Slot pressure is not an economic authorization to unwind the
            // entire strategy. Keep an underfunded dust queue uncommitted and
            // cancelable until it grows or honest idle becomes sufficient.
            revert RedeemCycleNotReady(outstandingRedeemShares, routingThreshold);
        }
    ...
    function _settleFromKnownIdle(bool strategyHealthy) internal {
        uint256 available = _spendableIdle();
        (uint256 payout, uint256 batchShare) = VaultBDepositLib.redeemSettlementPayout(
            redeemCycleAssetsSnapshot, redeemCycleCommittedShares, redeemCycleSupplySnapshot, available, strategyHealthy
        );
    ...
    function forceSettleStuckCycle() external nonReentrant {
        ...
        // A responsive strategy uses the healthy known-idle price; genuine
        // unavailability shares unrealizable deployed value pari passu.
    ...
    function commitThresholdShares() public view returns (uint256 threshold) {
        ...
        // Readiness and routing share the queue-open frozen basis. Live supply
        // growth cannot front-run an otherwise-ready permissionless commit.
        threshold = Math.ceilDiv(redeemCycleThresholdBase, 20);
    ...
    function claimRedeem(uint256 requestId) external nonReentrant returns (uint256 assets) {
        ...
        if (!redeemCycleForceSettled) {
            ...
            if (!released) {
                deferredRedeemHandles[request.strategyRequestId] = true;
                unchecked {
                    _deferredRedeemHandleCount += 1;
                }
        ...
        if (redeemCycleForceSettled) {
            burnedShares = VaultBDepositLib.coveredRedeemShares(
                shares,
                redeemCyclePayoutAssets,
                redeemCycleAssetsSnapshot,
                redeemCycleCommittedShares,
                redeemCycleSupplySnapshot
            );
    ...
    function claimableRedeemRequest(uint256 requestId) external view returns (uint256 assets) {
        ...
        return VaultBDepositLib.claimableRedeemRequest(
            VaultBDepositLib.ClaimableRedeemInput({
                asset: IERC20(asset()),
                vault: address(this),
                strategy: strategy,
                strategyRequestId: request.strategyRequestId,
                requestShares: request.shares,
                outstandingShares: outstandingRedeemShares,
                payoutAssets: redeemCyclePayoutAssets,
                payoutClaimed: redeemCyclePayoutClaimed,
                committedShares: redeemCycleCommittedShares,
                committedSupply: redeemCycleSupplySnapshot,
                assetsSnapshot: redeemCycleAssetsSnapshot,
                protocolCredit: redeemCycleProtocolCredit,
                claimableAssets: totalClaimableAssets,
                minimumResidualAssets: MIN_DEPOSIT,
                maximumLossBps: MAX_BATCH_EXECUTION_LOSS_BPS,
                forceSettled: redeemCycleForceSettled,
                localCommitted: _redeemCycleCommitted,
                settlementInitialized: redeemCycleSettlementInitialized
            })
        );
    }
    ...
    function _previewRedeemCycleSettlement(
        ...
        return VaultBDepositLib.previewRedeemCycleSettlement(
            activeStrategy,
            supply,
            batchShares,
            assetsSnapshot,

...
library VaultBDepositLib {
    ...
    function redeemBatchShare(uint256 assetsSnapshot, uint256 committedShares, uint256 supplySnapshot)
        external
        pure
        returns (uint256)
    {
        return Math.mulDiv(assetsSnapshot, committedShares, supplySnapshot);
    }
    ...
    function _previewRedeemCycleSettlement(
        ...
        if (supply - batchShares < minimumRedeemShares) return (true, currentAssets);
        ...
        uint256 maximum = Math.mulDiv(assetsSnapshot, maximumLossBps, 10_000);
    ...
    function initializeRedeemCycleSettlement(
        ...
        if (supply - batchShares < minimumRedeemShares) return (currentAssets, measured, charged);
...
/// A blacklisted synchronous receiver must retry to a clean address or use the
/// asynchronous path, whose failed push is escrowed without burning value.
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    function _commitRedeemCycle(IVaultBAsyncStrategy activeStrategy, uint256 threshold) internal {
        ...
        if (outstandingRedeemShares < routingThreshold) {
            ...
            uint256 batchShare = VaultBDepositLib.redeemBatchShare(
                redeemCycleAssetsSnapshot, redeemCycleCommittedShares, redeemCycleSupplySnapshot
            );
            if (available >= batchShare) {
    ...
    function forceSettleStuckCycle() external nonReentrant {
        ...
        uint256 requiredGas = FORCE_SETTLE_PROBE_GAS + MIN_FORCE_SETTLE_GAS_AFTER_PROBE;
        uint256 suppliedGas = gasleft();
        if (suppliedGas < requiredGas) revert InsufficientRecoveryGas(suppliedGas, requiredGas);
        (bool responsive, bytes memory response) = address(activeStrategy).staticcall{gas: FORCE_SETTLE_PROBE_GAS}(
            abi.encodeWithSelector(IVaultBAsyncStrategy.withdrawalCycleCommitted.selector)
        );
        bool strategyUnavailable = !responsive || response.length != 32;
    ...
    function claimableRedeemRequest(uint256 requestId) external view returns (uint256 assets) {
        ...
        return VaultBDepositLib.claimableRedeemRequest(
            VaultBDepositLib.ClaimableRedeemInput({
                ...
                minimumRedeemShares: MIN_REDEEM_SHARES,

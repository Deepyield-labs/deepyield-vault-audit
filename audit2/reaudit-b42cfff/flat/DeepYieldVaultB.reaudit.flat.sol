...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    function _settleFromKnownIdle(bool strategyHealthy) internal {
        ...
        if (redeemCycleSettlementInitialized) {
            ...
            // burn denominator. Paying only available idle then burns only the
            // covered fraction and returns the deployed claim as live shares.
            fullPayout = Math.mulDiv(redeemCycleAssetsSnapshot, redeemCycleCommittedShares, redeemCycleSupplySnapshot);
        }

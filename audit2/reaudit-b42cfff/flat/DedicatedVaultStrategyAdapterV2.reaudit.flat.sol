...
contract DedicatedVaultStrategyAdapterV2 is IVaultBAsyncStrategy, AccessControl, ReentrancyGuard {
    ...
    /// @notice Vault-only atomic migration handshake. It succeeds only after
    /// canonical Main is fully withdrawal-ready, has no queued handles and all
    /// direct shareholder backing has been returned to the Vault. A reverting
    /// valuation view cannot brick migration once these stronger custody facts
    /// are true; an active LP or incomplete inventory still fails closed.
    function prepareMigration() external onlyVault nonReentrant returns (bool prepared) {
        if (!main.isWithdrawalReady() || main.queuedWithdrawalCount() != 0) return false;
        _crystallizeFeeDirect();
        uint256 idle = asset.balanceOf(address(main));
        if (idle != 0) {
            uint256 withdrawn = _pullIdle(idle);
            asset.safeTransfer(vault, withdrawn);
        }
        if (asset.balanceOf(address(main)) != 0) return false;
        accountedAssets = 0;
        return true;
    }

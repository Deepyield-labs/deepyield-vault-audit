...
library VaultBDepositLib {
    ...
    function activateCandidate(
        ...
        if (address(oldStrategy) != address(0)) {
            ...
            (bool prepared, bool attested) = _callBool(address(oldStrategy), PREPARE_MIGRATION_SELECTOR);
            ...
            if (asset.balanceOf(oldAssetSource) != 0) revert StrategyNotEmpty();
            if (!attested) {
                (bool responsive, uint256 assets) =
                    _staticUint(address(oldStrategy), ESTIMATED_TOTAL_ASSETS_SELECTOR, 0);
                if (!responsive && !emergencyAllowed) revert StrategyNotEmpty();
                if (responsive && assets != 0) revert StrategyNotEmpty();
            }
            asset.forceApprove(address(oldStrategy), 0);
        }

        source = _validateCandidate(candidate, address(asset), vault);
        ...
        returns (bool)
    {
        ...
        try strategy.withdrawalCycleCommitted() returns (bool committed) {
            return committed;
        } catch {
            return false;
        }
    ...
    function cancelExpiredWithdrawal(
        ...
        if (_redeemCycleCommittedForExit(strategy, localCommitted)) revert RedeemCycleLocked();
        // The adapter may be the failed component while canonical Main has
        // already crossed its one-way boundary. Never convert that outage into
        // a loss-free cancellation merely because the adapter view reverted.
        bool directCommitted;
        try IVaultBDirectWithdrawalCycle(assetSource).withdrawalCycleCommitted() returns (bool committed) {
            directCommitted = committed;
        } catch {}
        if (directCommitted) revert RedeemCycleLocked();
        return !_cancelWithdrawalTolerant(strategy, assetSource, requestId);
    }

    /// @notice Tolerant canonical commitment proof plus the best independently
    /// observable NAV basis for a missing local snapshot. A responsive Strategy
    /// contributes its reported lower NAV; only a genuine Strategy/NAV outage
    /// falls back to spendable idle after Main proves the one-way commitment.
    function redeemCycleRecoverySnapshot() external view returns (bool committed, uint256 assetsSnapshot) {
        ...
        try strategy.withdrawalCycleCommitted() returns (bool strategyCommitted) {
            if (strategyCommitted) {
                return (true, _recoverySnapshotAssets(asset, address(p), strategy, reserved));
            }
        } catch {}
        try IVaultBDirectWithdrawalCycle(assetSource).withdrawalCycleCommitted() returns (bool directCommitted) {
            if (!directCommitted) return (false, 0);
        } catch {
            return (false, 0);
        }
        return (true, _recoverySnapshotAssets(asset, address(p), strategy, reserved));
    ...
    function _recoverySnapshotAssets(IERC20 asset, address vault, IVaultBAsyncStrategy strategy, uint256 reserved)
        private
        view
        returns (uint256 assetsSnapshot)
    {
        uint256 idle = asset.balanceOf(vault);
        assetsSnapshot = idle > reserved ? idle - reserved : 0;
        try strategy.estimatedTotalAssets() returns (uint256 deployed) {
            if (deployed <= type(uint256).max - idle) {
                uint256 gross = idle + deployed;
                assetsSnapshot = gross > reserved ? gross - reserved : 0;
            }
        } catch {}
    }

    function redeemCommitThreshold(uint256 thresholdBase, uint256 liveSupply, uint256 minimumShares)
        external
        pure
        returns (uint256)
    {
        return _commitThreshold(thresholdBase, liveSupply, minimumShares);
    }

    function _commitThreshold(uint256 thresholdBase, uint256 liveSupply, uint256 minimumShares)
        private
        pure
        returns (uint256 threshold)
    {
        uint256 basis = thresholdBase > liveSupply ? thresholdBase : liveSupply;
        threshold = Math.ceilDiv(basis, 20);
        if (threshold < minimumShares) threshold = minimumShares;
    }

    function strategyUnavailable(IVaultBAsyncStrategy strategy, uint256 probeGas, uint256 recoveryGas)
        external
        view
        returns (bool)
    {
        uint256 required = probeGas + recoveryGas;
    ...
    function _cancelWithdrawal(IVaultBAsyncStrategy strategy, address assetSource, bytes32 requestId) private {
        try strategy.cancelWithdrawal(requestId) returns (bool canceled) {
            if (canceled) return;
        } catch {}
        ...
        returns (bool released)
    {
        ...
        try IVaultBDirectForceSettlement(assetSource).forceClearWithdrawalFromVault(requestId) returns (bool cleared) {
            return cleared;
        } catch {
            return false;
        }
    ...
    function _callBool(address target, bytes4 selector) private returns (bool success, bool value) {
        uint256 result;
        uint256 size;
        assembly ("memory-safe") {
            let ptr := mload(0x40)
            mstore(ptr, selector)
            success := call(gas(), target, 0, ptr, 4, ptr, 32)
            size := returndatasize()
            result := mload(ptr)
        }
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    /// @notice Minimum supply basis frozen when the redeem queue opens. The live
    /// supply may raise, but never lower, the threshold: later capital cannot
    /// cheapen a vault-wide inventory unwind. Zero while the queue is empty.
    ...
    /// and distinct receivers get distinct slots. Slot hygiene plus the bounded
    /// queue and permissionless expiry limit, but do not eliminate, sybil fan-out.
    ...
    function applyStrategy() external onlyRole(ADMIN_ROLE) nonReentrant {
        ...
        _activateStrategy(next, _pendingStrategyAssetSource, paused());
    ...
    function _activateStrategy(address newStrategy, address expectedSource, bool emergencyAllowed) internal {
        if (newStrategy == address(0)) revert ZeroAddress();
        ...
        address source = VaultBDepositLib.activateCandidate(
            IERC20(asset()), address(this), strategy, strategyAssetSource, candidate, expectedSource, emergencyAllowed
        );
        strategy = candidate;
        ...
        returns (uint256 requestId)
    {
        ...
        uint256 cycleMaxPending = outstandingRedeemCount == 0 ? maxPendingRedeems : redeemCycleMaxPendingAtOpen;
        bool fillsQueue = outstandingRedeemCount + 1 >= cycleMaxPending;
        uint256 threshold;
        if (fillsQueue) {
            threshold = commitThresholdShares();
            if (outstandingRedeemShares + shares < threshold) {
                revert RedeemCycleNotReady(outstandingRedeemShares + shares, threshold);
            }
        }
    ...
    /// @notice Commit only after the batch reaches 5% of the greater of queue-open
    /// and live supply. There is deliberately no time-only path: small exits use
    /// the contract-enforced idle reserve and cannot trigger a vault-wide cycle.
    ...
    function _settleFromKnownIdle(bool strategyHealthy) internal {
        ...
        if (redeemCycleSettlementInitialized) {
            ...
            // entitlement; uncovered shares return through the force-claim path.
        ...
        } else if (strategyHealthy) {
            _initializeRedeemCycleSettlement(strategy);
            fullPayout = redeemCyclePayoutAssets;
        } else {
            fullPayout = Math.mulDiv(available, redeemCycleCommittedShares, redeemCycleSupplySnapshot);
        }
        uint256 payout = fullPayout < available ? fullPayout : available;
        // Once force settlement is fixed, the original snapshot has served its
        // cap function. Reuse this slot as the full live batch-payout denominator
        // so partial idle payment burns the matching share fraction.
        redeemCycleAssetsSnapshot = fullPayout;
    ...
    function forceSettleStuckCycle() external nonReentrant {
        ...
        bool strategyUnavailable = VaultBDepositLib.strategyUnavailable(
        ...
        _settleFromKnownIdle(!strategyUnavailable);
    }
    ...
    function commitThresholdShares() public view returns (uint256 threshold) {
        ...
        return VaultBDepositLib.redeemCommitThreshold(redeemCycleThresholdBase, totalSupply(), MIN_REDEEM_SHARES);
    }
    ...
    function claimRedeem(uint256 requestId) external whenNotPaused nonReentrant returns (uint256 assets) {
        ...
        uint256 burnedShares = shares;
        if (redeemCycleForceSettled) {
            burnedShares =
                VaultBDepositLib.coveredRedeemShares(shares, redeemCyclePayoutAssets, redeemCycleAssetsSnapshot);
            uint256 returnedShares = shares - burnedShares;
            if (returnedShares != 0) _transfer(address(this), request.owner, returnedShares);
        }
        ...
        emit Withdraw(msg.sender, request.receiver, request.owner, assets, burnedShares);

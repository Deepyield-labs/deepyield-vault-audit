...
library VaultBDepositLib {
    ...
    event EmergencyStrategyBackingQuarantined(address indexed source, uint256 assets);
    ...
    uint256 private constant MIGRATION_VIEW_GAS = 200_000;
    ...
    function activateCandidate(
        ...
        if (address(oldStrategy) != address(0)) {
            ...
            if (directBacking != 0) {
                ...
                if (!sourceWriteOffAllowed) {
                    // The first governed apply is a durable quarantine, not a
                    // failed transaction: revoke the old allowance and ask the
                    // Vault to advance its proposal clock by a second full delay.
                    asset.forceApprove(address(oldStrategy), 0);
                    emit EmergencyStrategyBackingQuarantined(oldAssetSource, directBacking);
                    return address(0);
                }
    ...
    function redeemQueueAdmission(
        ...
        uint256 newShares
    ) external pure returns (uint256 threshold, bool fillsQueue) {
        uint256 capacity = liveCap > frozenCap ? liveCap : frozenCap;
        if (count >= capacity) revert RedeemQueueFull();
        threshold = Math.ceilDiv(supply, 20);
        if (threshold < minimumShares) threshold = minimumShares;
        uint256 seatMinimum = Math.ceilDiv(threshold, capacity);
        if (seatMinimum < minimumShares) seatMinimum = minimumShares;
        if (newShares < seatMinimum) revert RedeemBelowMinimum(newShares, seatMinimum);
        fillsQueue = count + 1 >= capacity;
        if (fillsQueue && queuedShares + newShares < threshold) {
            revert RedeemCycleNotReady(queuedShares + newShares, threshold);
        }
    }
    ...
    /// observable full NAV basis for a missing local snapshot. A missing
    /// valuation fails closed instead of permanently latching idle-only NAV.
    function redeemCycleRecoverySnapshot() external view returns (bool committed, uint256 assetsSnapshot) {
        ...
        if (strategyResponsive && strategyCommitted != 0) {
            return _recoverySnapshotAssets(asset, address(p), strategy, reserved);
        }
        (bool directResponsive, uint256 directCommitted) =
            _staticUint(assetSource, IVaultBDirectWithdrawalCycle.withdrawalCycleCommitted.selector, COMMIT_PROBE_GAS);
        ...
        returns (bool navAvailable, uint256 assetsSnapshot)
    {
        ...
        // A missing local snapshot is an inconsistent recovery state in the
        // canonical callback graph. Never latch idle-only NAV as the permanent
        // execution-loss-cap basis; retry after the valuation view recovers.
    ...
    /// @notice Classify both coarse availability and complete healthy-settlement
    /// readiness in one bounded call. A responsive commitment selector alone
    /// does not prove that every pricing view used by initialization can be read.
    ...
    {
        _requireGas(probeGas * 5 + recoveryGas);
        ...
        if (committed == 0) return (false, false);
        (bool batchResponsive, uint256 batchCommitted) =
            _staticUint(address(strategy), IVaultBAsyncStrategy.withdrawalCycleBatchCommitted.selector, probeGas);
        if (!batchResponsive || batchCommitted == 0) return (false, false);
        (bool lossResponsive,) =
            _staticUint(address(strategy), IVaultBAsyncStrategy.withdrawalCycleExecutionLoss.selector, probeGas);
        if (!lossResponsive) return (false, false);
        (bool chargeResponsive,) = _staticUint(
            address(strategy), IVaultBAsyncStrategy.withdrawalCycleChargeableExecutionLoss.selector, probeGas
        );
        if (!chargeResponsive) return (false, false);
        (bool navResponsive,) = _staticUint(address(strategy), ESTIMATED_TOTAL_ASSETS_SELECTOR, probeGas);
        return (false, navResponsive);
    }
    ...
    function availableImmediateLiquidity(IERC20 asset, address vault, IVaultBAsyncStrategy strategy, uint256 reserved)
        external
        view
        returns (uint256)
    {
    ...
    /// @notice A known-idle force settlement realizes only part of the frozen
    /// batch NAV. Burn the same fraction of each request's escrowed shares and
    /// return the uncovered remainder to its owner; otherwise a zero-idle
    /// recovery would confiscate the entire deployed claim for zero assets.
    function coveredRedeemShares(uint256 requestShares, uint256 payoutAssets, uint256 fullBatchPayout)
        external
        pure
        returns (uint256 burnedShares)
    {
        if (fullBatchPayout == 0) return 0;
        burnedShares = Math.mulDiv(requestShares, payoutAssets, fullBatchPayout, Math.Rounding.Ceil);
        if (burnedShares > requestShares) burnedShares = requestShares;
    }
    ...
    function _previewRedeemCycleSettlement(
        ...
        uint256 residualAssets = Math.mulDiv(currentAssets, supply - batchShares, supply);
        // A genuine full-supply exit has no residual holder to protect from the
        // execution-loss cap. A merely sub-minimum residual still belongs to a
        // live holder, so it must pass the same cap before it may be swept.
        ...
        uint256 batchSnapshot = Math.mulDiv(assetsSnapshot, batchShares, supply);
        uint256 maximum = Math.mulDiv(batchSnapshot, maximumLossBps, 10_000);
        if (effective > maximum) return (false, 0);
        if (residualAssets < minimumResidualAssets) return (true, currentAssets);
    ...
    function initializeRedeemCycleSettlement(
        ...
        return _initializeRedeemCycleSettlement(
            ...
            currentAssets,
            protocolCredit,
            minimumResidualAssets,
            maximumLossBps
        );
    }

    /// @notice Report whether the ordinary settlement loss cap still admits the
    /// responsive cycle. A false result is consumed only by the delayed,
    /// guardian-paused force path; normal claims retain the strict reverting cap.
    function redeemCycleLossWithinCap(IVaultBAsyncStrategy strategy) external view returns (bool) {
        IVaultBRedeemState p = IVaultBRedeemState(address(this));
        ...
        uint256 credit = p.redeemCycleProtocolCredit();
        uint256 effective = measured > credit ? measured - credit : 0;
        uint256 batchSnapshot =
            Math.mulDiv(p.redeemCycleAssetsSnapshot(), p.redeemCycleCommittedShares(), p.redeemCycleSupplySnapshot());
        return effective <= Math.mulDiv(batchSnapshot, p.MAX_BATCH_EXECUTION_LOSS_BPS(), 10_000);
    }
    ...
    function _initializeRedeemCycleSettlement(
        ...
        if (residualAssets < minimumResidualAssets) return (currentAssets, measured, charged);
        uint256 basePayout = Math.mulDiv(currentAssets, batchShares, supply);
        uint256 charge = Math.mulDiv(charged, supply - batchShares, supply, Math.Rounding.Ceil);
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    /// ADMIN_ROLE/GUARDIAN_ROLE grants by the current default admin remain immediate
    /// OpenZeppelin AccessControl operations; strategy replacement has its own
    /// independent STRATEGY_TIMELOCK. Hard-wired so the constructor ABI is unchanged.
    ...
    /// false unavailability result while retaining ample gas for local settlement.
    ...
    constructor(
        ...
        // delayed admin cannot collapse dual-control recovery gates in one block.
    ...
    function setStrategy(address newStrategy) external onlyRole(ADMIN_ROLE) {
        ...
        _activateStrategy(newStrategy, address(0), false);
    }
    ...
    function applyStrategy() external onlyRole(ADMIN_ROLE) nonReentrant {
        ...
        if (_activateStrategy(next, _pendingStrategyAssetSource, paused())) _clearStrategyProposal();
    }
    ...
    function _activateStrategy(address newStrategy, address expectedSource, bool emergencyAllowed)
        internal
        returns (bool activated)
    {
        ...
        address source = VaultBDepositLib.activateCandidate(
            ...
            expectedSource,
            emergencyAllowed,
            _emergencyStrategySourceWriteOffScheduled
        );
        if (source == address(0)) {
    ...
    function maxDepositStrict(address) external view returns (uint256) {
        return VaultBDepositLib.maxDepositStrict(
            ...
            paused() || redeemCycleCommitted() || pendingStrategy != address(0)
                || (outstandingRedeemCount != 0 && outstandingRedeemCount >= redeemCycleMaxPendingAtOpen),
            totalSupply(),
            depositCap,
        ...
        returns (uint256 requestId)
    {
        ...
        (uint256 threshold, bool fillsQueue) = VaultBDepositLib.redeemQueueAdmission(
            totalSupply(),
            MIN_REDEEM_SHARES,
            maxPendingRedeems,
            redeemCycleMaxPendingAtOpen,
            outstandingRedeemCount,
            outstandingRedeemShares,
            shares
        );
        ...
        // The final slot is admitted only for an economic batch and commits in
        // this transaction. Any downstream failure rolls the request and its
        // Main registration back, leaving the previous cancelable queue intact.
        if (fillsQueue) _commitRedeemCycle(activeStrategy, threshold);
    }
    ...
    /// @param strategyHealthy true for guardian-authorized recovery from a responsive
    /// cycle, false when the strategy is unavailable.
    ...
    ///  - Unavailable/over-cap strategy: the batch receives its supply-pro-rata
    ///    share of known idle plus any earmarked protocol credit, then burns its
    ///    escrow in full. Returning fungible shares after that payout would let
    ///    the batch reclaim the same remaining idle a second time.
    function _settleFromKnownIdle(bool strategyHealthy) internal {
        uint256 available = _spendableIdle();
        uint256 fullPayout;
        if (redeemCycleSettlementInitialized) {
            // A prior claim fixed the cycle price but did not realize every
            // request. Rebase onto only the outstanding shares and their unpaid
            // entitlement.
            fullPayout = redeemCyclePayoutAssets - redeemCyclePayoutClaimed;
            redeemCycleCommittedShares = outstandingRedeemShares;
            redeemCyclePayoutClaimed = 0;
            if (!strategyHealthy) {
                available = Math.mulDiv(available, redeemCycleCommittedShares, totalSupply());
            }
        } else if (strategyHealthy && VaultBDepositLib.redeemCycleLossWithinCap(strategy)) {
            _initializeRedeemCycleSettlement(strategy);
            fullPayout = redeemCyclePayoutAssets;
        } else {
            // The delayed guardian path may reach this branch because the
            // strategy is unavailable OR because a responsive realized loss
            // exceeded the ordinary cap. Only known idle is payable. Treasury
            // credit is batch-earmarked and therefore added after, not inside,
            // the supply-pro-rata idle allocation.
            ...
            fullPayout = Math.mulDiv(redeemCycleAssetsSnapshot, batchShares, supply) + credit;
            uint256 shareholderIdle = available > credit ? available - credit : 0;
            available = Math.mulDiv(shareholderIdle, batchShares, supply) + credit;
        }
        uint256 payout = available;
        if (fullPayout < available) payout = fullPayout;
        // Keep the original NAV snapshot as provenance. Claims use only the
        // force-settled payout below and burn their complete escrow because that
        // payout is the batch's full pro-rata claim on presently-provable value.
        redeemCyclePayoutAssets = payout;
        redeemCycleSettlementInitialized = true;
        redeemCycleForceSettled = true;
        emit RedeemCycleForceSettled(payout, fullPayout, available);
    }
    ...
    /// The force payout uses only known spendable idle, pro-rata to the batch's
    /// frozen share of supply, and never spends a stale claim on unreadable capital.
    function forceSettleStuckCycle() external nonReentrant {
        ...
        if (redeemCycleForceSettled) revert RedeemCycleAlreadySettled();
        ...
        (bool strategyUnavailable, bool settlementHealthy) = VaultBDepositLib.strategyRecoveryStatus(
        ...
        // A responsive uninitialized cycle uses the healthy known-idle price;
        // genuine unavailability shares unrealizable deployed value pari passu.
        // An initialized residual keeps its already-frozen entitlement instead.
        _settleFromKnownIdle(settlementHealthy);
    }
    ...
    function claimRedeem(uint256 requestId) external nonReentrant returns (uint256 assets) {
        ...
        // A known-idle force payout is already the batch's full pro-rata claim on
        // the only presently provable assets. Returning fungible shares would let
        // them claim the same remaining idle a second time, so the escrow burns in full.

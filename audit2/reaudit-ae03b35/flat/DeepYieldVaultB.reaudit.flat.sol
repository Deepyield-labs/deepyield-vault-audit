...
library VaultBDepositLib {
    ...
    function activateCandidate(
        ...
        if (address(oldStrategy) != address(0)) {
            ...
            (bool responsive, uint256 assets) = _staticUint(address(oldStrategy), ESTIMATED_TOTAL_ASSETS_SELECTOR, 0);
        ...
        returns (bool)
    {
        ...
        // mutation must observe that independent pinned-source witness too.
        address assetSource = IVaultBRedeemState(address(this)).strategyAssetSource();
        if (assetSource == address(0)) return false;
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
        ...
        returns (uint256 requestId)
    {
        ...
        if (existingPlusOne != 0) {
            ...
            // A top-up renews the whole aggregated intent. Otherwise a fresh,
            // larger addition inherits an old slot's permissionless expiry.
            // forge-lint: disable-next-line(unsafe-typecast)
            existing.requestedAt = uint64(block.timestamp);
    ...
    ///  - Unavailable strategy: the frozen batch entitlement remains the burn
    ///    denominator. Available idle pays and burns only its covered fraction;
    ///    uncovered shares return with their claim on later-recovered capital.
    function _settleFromKnownIdle(bool strategyHealthy) internal {
        ...
        if (redeemCycleSettlementInitialized) {
            ...
            // Preserve the unavailable batch's frozen full entitlement as the
            // burn denominator. It may draw only its supply-pro-rata share of
            // known idle; the uncovered deployed claim returns as live shares.
            uint256 batchShares = redeemCycleCommittedShares;
            uint256 supply = redeemCycleSupplySnapshot;
            fullPayout = Math.mulDiv(redeemCycleAssetsSnapshot, batchShares, supply);
            available = Math.mulDiv(available, batchShares, supply);
        }
    ...
    function _redeemKey(address owner, address receiver) internal pure returns (bytes32 key) {
        assembly ("memory-safe") {
            mstore(0x00, owner)

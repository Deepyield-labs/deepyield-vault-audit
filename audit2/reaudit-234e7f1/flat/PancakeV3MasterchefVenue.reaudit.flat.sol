...
contract PancakeV3MasterchefVenue is IDedicatedVenueV2, ERC721Holder {
    ...
    function open(OpenArgs calldata a) external onlyController returns (uint256 tokenId) {
        ...
        uint256 amount0Used;
        uint256 amount1Used;
        (tokenId,, amount0Used, amount1Used) = nfpm.mint(
        ...
        // M2 (Audit 2, Main Unified): return ONLY the unused approved legs to the
        // controller — the exact `(assetAmount − amount0Used, pairedAmount − amount1Used)`
        // the mint left over. Do NOT sweep the venue's whole balance here: a third party
        // can `transfer` a donation to this fixed address, and sweeping it into Main
        // corrupts Main's raw-balance-delta consumption measure in `MainV2Open.openPosition`
        // (understating `deployedValue` by the donation, or underflowing), which could
        // permanently brick the open path with no active position to recover through. The
        // mint reports exactly what it consumed, so the surplus is exact and donation-free;
        // any donation stays inert at the venue and is swept to Main on the next sweep
        // (harvest or close). Main's canonical-vs-raw accounting (recognizedBalances =
        // min(raw, accounted)) then keeps a swept donation from being extractable value.
        uint256 assetRefund = a.assetAmount - amount0Used;
        uint256 pairedRefund = a.pairedAmount - amount1Used;
        if (assetRefund != 0) asset.safeTransfer(controller, assetRefund);
        if (pairedRefund != 0) paired.safeTransfer(controller, pairedRefund);
    }

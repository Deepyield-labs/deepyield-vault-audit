...
contract RichGuard is IVaultBPriceGuard, IVaultBRewardPriceGuard {
    ...
    function minimumOutAndBudget(address tokenIn, address, uint256 amountIn, bool)
        external
        view
        returns (uint256 minOut, uint256 emergencyNotional, bool emergencyBudgetUsed)
    {
        return (_quote(tokenIn, amountIn) * (10_000 - lossBps) / 10_000, amountIn, false);
    }
    ...
    function minimumOutAndBudget(uint256 amountIn, bool)
        external
        view
        returns (uint256 minOut, uint256 emergencyNotional, bool emergencyBudgetUsed)
    {
        return (amountIn * (10_000 - lossBps) / 10_000, amountIn, false);
    }

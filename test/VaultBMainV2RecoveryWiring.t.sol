...
contract VaultBMainV2RecoveryWiringTest is Test {
    ...
    function _openPosition(bytes32 jobId) internal returns (uint256 id) {
        ...
        vm.prank(keeper);
        main.reserveOpenSeries(jobId, 1_000e18, 500e18, -100, 100, block.timestamp + 60); // B11-T1
        vm.prank(keeper);
    ...
    // ── B9-T2: the staged-recovery entrypoints commit the redeem batch before
    //          the first irreversible venue step, so a queued request cannot be
    //          cancelled after the emergency close begins. ────────────────────

    function test_B9T2_RecoverUnstakeCommitsCycleBlockingCancel() public {
        _openPosition();
        main.requestWithdrawal(keccak256("b9t2-req"), 500e18); // this contract is the vault
        assertFalse(main.withdrawalCycleCommitted());

        vm.prank(guardian);
        main.recoverCloseUnstake();
        assertTrue(main.withdrawalCycleCommitted(), "unstake fixes the batch before the irreversible step");

        // The queued request can no longer be cancelled out of the committed batch.
        vm.expectRevert(DedicatedVaultMainV2.WithdrawalBatchCommitted.selector);
        main.cancelWithdrawal(keccak256("b9t2-req"));
    }

    function test_B9T2_WriteOffCommitsCycleBlockingCancel() public {
        _openPosition();
        main.requestWithdrawal(keccak256("b9t2-req2"), 500e18);

        vm.prank(guardian);
        main.writeOffStrandedPosition();
        assertTrue(main.withdrawalCycleCommitted(), "write-off fixes the batch");

        vm.expectRevert(DedicatedVaultMainV2.WithdrawalBatchCommitted.selector);
        main.cancelWithdrawal(keccak256("b9t2-req2"));
    }

    // ── B9-T2 (requirement 3): the emergency path stays passable with an empty
    //          queue and while HALTED — the commit is a no-op, not a new gate. ──
    function test_B9T2_RecoveryPassableWithEmptyQueueWhileHalted() public {
        _openPosition();
        vm.prank(guardian);
        main.halt();
        ...
        vm.prank(guardian);
        main.recoverCloseUnstake(); // must not revert; nothing to commit
        assertFalse(main.withdrawalCycleCommitted(), "empty queue: nothing committed");
    }
}

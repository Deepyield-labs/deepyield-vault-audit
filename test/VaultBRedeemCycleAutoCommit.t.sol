...
contract VaultBRedeemCycleAutoCommitTest is VaultBAsyncRedeemV2Test {
    ...
    function testAutoCommittedOverCapLossEnforcesCapThenTreasurySettles() public {
        ...
        // The reverted claim rolled back its lazy snapshot; take it explicitly (п.4)
        // so the cap base is readable, then fund exactly the over-cap deficit (п.3).
    ...
    function testTwoQueuedRequestsSettleEquallyDespiteNavDriftBetweenClaims() public {
        ...
        assertApproxEqAbs(carolAssets, bobAssets, 1, "equal requests settle equally; late claimer gets no NAV-drift edge");
    }

    // ---- п.4 : commitRedeemCycle snapshots after auto-commit, not reverts ----

    /// After Main auto-commits, an explicit `commitRedeemCycle` must take the
    /// batch snapshot (so the treasury can fund and settlement can run) instead of
    /// reverting on the strategy's one-way door, and it must not re-commit the
    /// strategy. Old code reverted `RedeemCycleLocked` here.
    function testCommitRedeemCycleSnapshotsAfterAutoCommit() public {
        uint256 totalShares = _depositAndDeploy(alice, 1_000e18);
        uint256 queued = (totalShares + 19) / 20;
        vm.prank(alice);
        vault.transfer(bob, queued);
        ...
        assertTrue(vault.redeemCycleCommitted(), "combined view already reflects the auto-commit");
        assertEq(vault.redeemCycleCommittedShares(), 0, "local snapshot not yet materialized");

        // Succeeds (would revert on the old code) and re-committing the strategy is
        // skipped — if it were re-committed, Main would revert and take this call.
        vault.commitRedeemCycle();
        assertEq(vault.redeemCycleCommittedShares(), queued, "snapshot fixes the committed shares");
